...
/// Docs: https://www.dropbox.com/developers/documentation/http/documentation#file_properties-properties-overwrite
...
impl Utils for PropertiesOverwriteRequest<'_> {
...
impl Service<PropertiesOverwriteResponse, BoxFuture<'_, Result<PropertiesOverwriteResponse>>>
    for PropertiesOverwriteRequest<'_>
{
    fn call(
        &self,
        ...
        let block = async {
            ...
            let response: PropertiesOverwriteResponse = response
                .json()
                .await
                .map_err(|err| ApiError::ParsingError(err.into()))?;

            Result::<PropertiesOverwriteResponse>::Ok(response)
        };
        Ok(Box::pin(block))
    }
...
mod tests {
    ...
    use super::{Field, PropertiesOverwriteRequest, PropertiesOverwriteResponse, TemplateID};
    // #[tokio::test]
    ...
    pub fn test_properties_generation() -> Result<()> {
        ...
        let request = PropertiesOverwriteRequest {
            access_token: "123",
            path: "123",
