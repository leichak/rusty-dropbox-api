...
impl Utils for PropertiesSearchRequest<'_> {
...
impl Service<PropertiesSearchResponse, BoxFuture<'_, Result<PropertiesSearchResponse>>>
    for PropertiesSearchRequest<'_>
{
    fn call(
        &self,
    ) -> Result<Pin<Box<dyn Future<Output = Result<PropertiesSearchResponse>> + Send>>> {
        let endpoint = Endpoint::PropertiesSearchPost.get_endpoint_url();
        ...
        let block = async {
            ...
            let response: PropertiesSearchResponse = response
                .json()
                .await
                .map_err(|err| ApiError::ParsingError(err.into()))?;

            Result::<PropertiesSearchResponse>::Ok(response)
        };
        Ok(Box::pin(block))
    }
    fn call_sync(&self) -> Result<PropertiesSearchResponse> {
        let endpoint = Endpoint::PropertiesSearchPost.get_endpoint_url();
...
mod tests {
    ...
    use super::{Field, PropertiesSearchRequest, PropertiesSearchResponse, TemplateID};
    // #[tokio::test]
    ...
    pub fn test_properties_generation() -> Result<()> {
        ...
        let request = PropertiesSearchRequest {
            access_token: "123",
            path: "123",
