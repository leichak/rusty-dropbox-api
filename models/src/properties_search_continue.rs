...
impl Utils for PropertiesSearchContinueRequest<'_> {
...
impl
    Service<
        ...
        BoxFuture<'_, Result<PropertiesSearchContinueResponse>>,
    > for PropertiesSearchContinueRequest<'_>
{
    fn call(
        &self,
    ) -> Result<Pin<Box<dyn Future<Output = Result<PropertiesSearchContinueResponse>> + Send>>>
    {
        ...
        let block = async {
            ...
            let response: PropertiesSearchContinueResponse = response
                .json()
                .await
                .map_err(|err| ApiError::ParsingError(err.into()))?;

            Result::<PropertiesSearchContinueResponse>::Ok(response)
        };
        Ok(Box::pin(block))
    }
...
mod tests {
    ...
    use super::{
        Field, PropertiesSearchContinueRequest, PropertiesSearchContinueResponse, TemplateID,
    };
    // #[tokio::test]
    ...
    pub fn test_properties_generation() -> Result<()> {
        ...
        let request = PropertiesSearchContinueRequest {
            access_token: "123",
            path: "123",
