...
impl Utils for PropertiesRemoveRequest<'_> {
...
impl Service<PropertiesRemoveResponse, BoxFuture<'_, Result<PropertiesRemoveResponse>>>
    for PropertiesRemoveRequest<'_>
{
    fn call(
        &self,
    ) -> Result<Pin<Box<dyn Future<Output = Result<PropertiesRemoveResponse>> + Send>>> {
        let endpoint = Endpoint::PropertiesRemovePost.get_endpoint_url();
        ...
        let block = async {
            ...
            let response: PropertiesRemoveResponse = response
                .json()
                .await
                .map_err(|err| ApiError::ParsingError(err.into()))?;

            Result::<PropertiesRemoveResponse>::Ok(response)
        };
        Ok(Box::pin(block))
    }
    fn call_sync(&self) -> Result<PropertiesRemoveResponse> {
        let endpoint = Endpoint::PropertiesRemovePost.get_endpoint_url();
...
mod tests {
    ...
    use super::{Field, PropertiesRemoveRequest, PropertiesRemoveResponse, TemplateID};
    // #[tokio::test]
    ...
    pub fn test_properties_generation() -> Result<()> {
        ...
        let request = PropertiesRemoveRequest {
            access_token: "123",
            path: "123",
