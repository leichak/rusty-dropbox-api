...
impl Utils for TemplatesAddForUserRequest<'_> {
...
impl Service<TemplatesAddForUserResponse, BoxFuture<'_, Result<TemplatesAddForUserResponse>>>
    for TemplatesAddForUserRequest<'_>
{
    fn call(
        &self,
    ) -> Result<Pin<Box<dyn Future<Output = Result<TemplatesAddForUserResponse>> + Send>>> {
        let endpoint = Endpoint::TemplatesAddForUserPost.get_endpoint_url();
...
mod tests {
    ...
    use super::{Field, TemplateID, TemplatesAddForUserRequest, TemplatesAddForUserResponse};
    // #[tokio::test]
