...
impl Utils for TemplatesGetForUserRequest<'_> {
...
impl Service<TemplatesGetForUserResponse, BoxFuture<'_, Result<TemplatesGetForUserResponse>>>
    for TemplatesGetForUserRequest<'_>
{
    fn call(
        &self,
    ) -> Result<Pin<Box<dyn Future<Output = Result<TemplatesGetForUserResponse>> + Send>>> {
        let endpoint = Endpoint::TemplatesGetForUserPost.get_endpoint_url();
        ...
        let block = async {
            ...
            let response: TemplatesGetForUserResponse = response
                .json()
                .await
                .map_err(|err| ApiError::ParsingError(err.into()))?;

            Result::<TemplatesGetForUserResponse>::Ok(response)
        };
        Ok(Box::pin(block))
    }
    fn call_sync(&self) -> Result<TemplatesGetForUserResponse> {
        let endpoint = Endpoint::TemplatesGetForUserPost.get_endpoint_url();
...
mod tests {
    ...
    use super::{Field, TemplateID, TemplatesGetForUserRequest, TemplatesGetForUserResponse};
    // #[tokio::test]
    ...
    pub fn test_properties_generation() -> Result<()> {
        ...
        let request = TemplatesGetForUserRequest {
            access_token: "123",
            path: "123",
