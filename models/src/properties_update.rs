...
impl Utils for PropertiesUpdateRequest<'_> {
...
impl Service<PropertiesUpdateResponse, BoxFuture<'_, Result<PropertiesUpdateResponse>>>
    for PropertiesUpdateRequest<'_>
{
    fn call(
        &self,
    ) -> Result<Pin<Box<dyn Future<Output = Result<PropertiesUpdateResponse>> + Send>>> {
        let endpoint = Endpoint::PropertiesUpdatePost.get_endpoint_url();
        ...
        let block = async {
            ...
            let response: PropertiesUpdateResponse = response
                .json()
                .await
                .map_err(|err| ApiError::ParsingError(err.into()))?;

            Result::<PropertiesUpdateResponse>::Ok(response)
        };
        Ok(Box::pin(block))
    }
    fn call_sync(&self) -> Result<PropertiesUpdateResponse> {
        let endpoint = Endpoint::PropertiesUpdatePost.get_endpoint_url();
...
mod tests {
    ...
    use super::{Field, PropertiesUpdateRequest, PropertiesUpdateResponse, TemplateID};
    // #[tokio::test]
    ...
    pub fn test_properties_generation() -> Result<()> {
        ...
        let request = PropertiesUpdateRequest {
            access_token: "123",
            path: "123",
